---
description: "소대 편제 방식 팀구성 (42개~) - 소대장+연락병+N분대(Alpha/Bravo/Charlie/Delta/Echo/...) + 용병4 + 18 Skills"
user-invocable: true
---

# Platoon Formation (소대 편제 방식 팀구성)

현재 프로젝트 디렉토리 이름을 기반으로 Agent Teams 소대(42개 이상)를 편성하고, 용병(CLI/API/MCP)을 전략적으로 활용하여 임무를 수행한다.
...
| 분대원 (SubAgent) | N×11 | haiku / sonnet | 11개/분대, 분대장이 작업 복잡도 판단하여 선정 |
| 용병 (외부 AI) | 4 | 타사 모델 | 공유 풀, 특화 작업 |
| **합계** | **42개~** | | |

**편제 공식:** HQ(2) + N Squads × 12(Leader+11) + 4 Mercenaries(Shared) (N≥3)
...
### Step 2-1: 표준 분대원 11개 (각 분대 공통)

각 분대에 11개의 분대원을 배치한다. 분대장이 Task 도구로 투입.
...
편제: HQ(2) + N × 12(Leader+11) + 4 Mercenaries = {총원}개 (N≥3)
...
2. Alpha, Bravo, Charlie (+ Delta, Echo) 스폰 (sonnet 모델, 각 11개 분대원)
3. 편제 보고 (42개 이상)
...
1분대 (12개): Alpha + 분대원 11개
2분대 (12개): Bravo + 분대원 11개
3분대 (12개): Charlie + 분대원 11개
4분대 (12개): Delta + 분대원 11개    ← 확장 시
5분대 (12개): Echo + 분대원 11개     ← 확장 시

용병 풀 (4개, 공유): Codex / Gemini / Grok / Perplexity
```
